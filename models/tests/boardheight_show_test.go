...
func TestBoardHeightShow(t *testing.T) {
	...
	testBoardHeight := models.BoardHeight{Height: 32}
	db.Database.Create(&testBoardHeight)

	// Test BoardHeightShow with valid ID
	boardHeight, err := models.BoardHeightShow(uint64(testBoardHeight.ID))
	...
	_, err = models.BoardHeightShow(0) // ID 0 is invalid
	assert.Error(t, err)
