...
func TestCheckEmailUsernameAvailable(t *testing.T) {
	...
	available, err := models.CheckEmailUsernameAvailable("test@example.com", "differentusername")
	assert.NoError(t, err)
	assert.False(t, available, "Email should not be available")

	// Test with an unavailable email and username
	available, err = models.CheckEmailUsernameAvailable("differentemail@example.com", "testuser")
	assert.NoError(t, err)
	assert.False(t, available, "Username should not be available")

	// Test with an unavailable email and username
	available, err = models.CheckEmailUsernameAvailable("test@example.com", "testuser")
	assert.NoError(t, err)
	assert.False(t, available, "Email and username should not be available")

	// Test with available email and username
	available, err = models.CheckEmailUsernameAvailable("differentemail@example.com", "differentusername")
	assert.NoError(t, err)
