...
// TestTeamTypeCreate is a controller for creating a new team_type
func TestTeamTypeCreate(t *testing.T) {
	...
	r.POST("/admin/teamtypes", controllers.TeamTypeCreate)

	// Create a post request with form data
	data := url.Values{}
	data.Set("name", "TestTeamType")
	req, err := http.NewRequest("POST", "/admin/teamtypes", strings.NewReader(data.Encode()))
	if err != nil {
		t.Fatal(err)
	}
	...
	// Check the database for the new team_type
	var teamType models.TeamType
	if err := db.Database.Where("name = ?", "TestTeamType").First(&teamType).Error; err != nil {
		t.Errorf("handler did not create a new team_type: %v", err)
	}
