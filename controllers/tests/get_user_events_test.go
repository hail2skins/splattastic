...
// TestGetUserEvents tests the GetUserEvents controller
func TestGetUserEvents(t *testing.T) {
	...
	// Test cases struct allows multiple tests to be run with the same code
	testCases := []struct {
		name          string
		diveIDs       []uint64
		expectedDives int
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			req, err := http.NewRequest("GET", "/user/"+helpers.UintToString(user.ID)+"/events", nil)
			if err != nil {
			...
			if !strings.Contains(body, event.Name) {
				t.Errorf("Expected body to contain %s, got %s", event.Name, body)
			}
			if !strings.Contains(body, "Nothing") {
				t.Errorf("Expected body to contain %s, got %s", event1.Name, body)
			}
