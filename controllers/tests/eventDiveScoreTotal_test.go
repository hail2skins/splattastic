...
// TestEventDiveScoreTotal tests the eventDiveScoreTotal controller
func TestEventDiveScoreTotal(t *testing.T) {
	// Setup
	LoadEnv()
	db.Connect()
	...
	// Route Get
	router.GET("/user/:id/event/:event_id/dive/:dive_id/total", func(c *gin.Context) {
		userID, _ := strconv.ParseUint(c.Param("id"), 10, 64)
		c.Set("user_id", uint(userID))
		controllers.EventDiveScoreTotal(c)
	})
	...
	for _, tc := range testCases {
		...
		// Test EventDiveScoreTotal
		req, err := http.NewRequest("GET", "/user/"+strconv.FormatUint(uint64(user.ID), 10)+"/event/"+strconv.FormatUint(uint64(event.ID), 10)+"/dive/"+strconv.FormatUint(uint64(dive.ID), 10)+"/total", nil)
		if err != nil {
			t.Fatalf("Error creating request: %v", err)
		}
		// Response recorder for inspecting the response
		rr := httptest.NewRecorder()
		// Serve the request to the recorder
		router.ServeHTTP(rr, req)

		if tc.expectErr && rr.Code != http.StatusInternalServerError {
			t.Fatalf("Expected error calculating dive score with %d scores", tc.numScores)
		} else if !tc.expectErr && rr.Code != http.StatusOK {
			t.Fatalf("Unexpected error calculating dive score with %d scores: %v", tc.numScores, err)
		}
	}
