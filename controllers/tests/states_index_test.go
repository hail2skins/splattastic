...
// TestStatesIndex tests the StatesIndex function
func TestStatesIndex(t *testing.T) {
	// Setup
	LoadEnv()
	db.Connect()
	...
	r.GET("/admin/states", controllers.StatesIndex)

	// Create two states
	state1, _ := models.StateCreate("Test State", "TS")
	state2, _ := models.StateCreate("Test Again", "TA")

	// Create a get request to /admin/states
	req, err := http.NewRequest("GET", "/admin/states", nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	// Check the response body for state 1
	expected := state1.Name
	if !strings.Contains(rr.Body.String(), expected) {
		t.Errorf("handler returned unexpected body: got %v want %v", rr.Body.String(), expected)
	}
	// Check the response body for state 2
	expected = state2.Name
	if !strings.Contains(rr.Body.String(), expected) {
		t.Errorf("handler returned unexpected body: got %v want %v", rr.Body.String(), expected)
	}

	// Check the response body for state 1 code
	expected = state1.Code
	if !strings.Contains(rr.Body.String(), expected) {
		t.Errorf("handler returned unexpected body: got %v want %v", rr.Body.String(), expected)
	}
	// Check the response body for state 2 code
	expected = state2.Code
	if !strings.Contains(rr.Body.String(), expected) {
		t.Errorf("handler returned unexpected body: got %v want %v", rr.Body.String(), expected)
	}

	// Check the response body for the header
	expected = "States"
	if !strings.Contains(rr.Body.String(), expected) {
		t.Errorf("handler returned unexpected body: got %v want %v", rr.Body.String(), expected)
	}
