...
func TestTeamTypeDelete(t *testing.T) {
	...
	r.DELETE("/admin/teamtypes/:id", controllers.TeamTypeDelete)

	// Create a teamtype
	tt1, _ := models.TeamTypeCreate("TestTeamType1")

	// Create a delete request to /admin/teamtypes/:id
	req, err := http.NewRequest("DELETE", "/admin/teamtypes/"+helpers.UintToString(tt1.ID), nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	// Check that the teamtype was deleted
	_, err = models.TeamTypeShow(uint64(tt1.ID))
	if err == nil {
		t.Errorf("TeamType was not deleted")
	}
